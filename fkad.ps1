...
# fkmde
try {
    Write-Host "[*] fkmde (Defender)..." -ForegroundColor DarkGray
    Invoke-Expression (New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/fkxdr/fkmde/refs/heads/main/fkmde.ps1') | Out-File "$OUT\fkmde.txt" -Encoding utf8
    Write-Host "[OK] fkmde -> fkmde.txt" -ForegroundColor Green
} catch { Write-Host "[--] fkmde failed: $_" -ForegroundColor DarkGray }

# PrivescCheck
try {
    Write-Host "[*] PrivescCheck..." -ForegroundColor DarkGray
...
# WinPEAS
try {
    Write-Host "[*] WinPEAS..." -ForegroundColor DarkGray
    IEX (iwr 'https://raw.githubusercontent.com/peass-ng/PEASS-ng/master/winPEAS/winPEASps1/winPEAS.ps1') | Out-File "$OUT\winpeas.txt" -Encoding utf8
    Write-Host "[OK] WinPEAS -> winpeas.txt" -ForegroundColor Green
} catch { Write-Host "[--] WinPEAS failed: $_" -ForegroundColor DarkGray }

# HardeningKitty
try {
    Write-Host "[*] HardeningKitty..." -ForegroundColor DarkGray
...
# ScriptSentry
try {
    Write-Host "[*] ScriptSentry..." -ForegroundColor DarkGray
    IEX (iwr 'https://raw.githubusercontent.com/techspence/ScriptSentry/main/ScriptSentry.ps1') | Out-File "$OUT\scriptsentry.txt" -Encoding utf8
...
# AppLocker Inspector
try {
    Write-Host "[*] AppLocker Inspector..." -ForegroundColor DarkGray
...
# PowerUpSQL
try {
    Write-Host "[*] PowerUpSQL (MSSQL)..." -ForegroundColor DarkGray
